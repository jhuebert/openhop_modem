...
std::string renderRootPage(const Model& m) {
    ...
    if (m.capabilities.wifi) {
        body += "<div class='checkline'><input type='checkbox' id='wifi_ps' name='wifi_ps' value='1'" + std::string(m.config.wifiPowerSave ? " checked" : "") + "><label for='wifi_ps'>Wi-Fi power save (disable for lower latency, higher power draw)</label></div>";
    }
    ...
    if (m.capabilities.writableManagement) {
        body += "<div id='save-status' class='summary' hidden></div><script>(()=>{for(const form of document.querySelectorAll('form[data-reboot-save]')){form.addEventListener('submit',async event=>{event.preventDefault();const fields=new FormData(form);let target=location.origin+'/';if(form.dataset.rebootSave==='network'&&fields.has('static')){const ip=String(fields.get('ip')||'').trim();if(ip)target=location.protocol+'//'+ip+'/';}const status=document.getElementById('save-status');status.hidden=false;status.textContent='Saving settings...';for(const button of form.querySelectorAll('button'))button.disabled=true;try{const response=await fetch(form.action,{method:'POST',body:new URLSearchParams(new FormData(form)),credentials:'same-origin'});const message=await response.text();if(!response.ok)throw new Error(message||('HTTP '+response.status));status.textContent='Settings saved. Modem is rebooting. Returning to the main page...';setTimeout(()=>location.replace(target),7000);}catch(error){status.textContent='Save failed: '+error.message;for(const button of form.querySelectorAll('button'))button.disabled=false;}});}})();</script>";
    }
    body += "</body></html>";
    return body;
}
