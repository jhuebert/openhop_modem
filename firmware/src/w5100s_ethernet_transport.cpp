...
#endif
#ifndef OPENHOP_ETH_DHCP_RETRY_TIMEOUT_MS
#  define OPENHOP_ETH_DHCP_RETRY_TIMEOUT_MS 250UL
#endif
#ifndef OPENHOP_ETH_DHCP_RETRY_RESPONSE_TIMEOUT_MS
#  define OPENHOP_ETH_DHCP_RETRY_RESPONSE_TIMEOUT_MS 100UL
#endif
...
namespace EthernetManager {
    ...
    static char hostnameString[65] = OPENHOP_ETH_HOSTNAME;
    ...
    static bool tryDhcp(const char* reason,
                        unsigned long timeoutMs = OPENHOP_ETH_DHCP_TIMEOUT_MS,
                        unsigned long responseTimeoutMs = OPENHOP_ETH_DHCP_RESPONSE_TIMEOUT_MS) {
...
#  endif
    ...
    void loop() {
        ...
        if (dhcpMode && !gotIP && linkStatusIsUp() &&
            ...
            tryDhcp("retry", OPENHOP_ETH_DHCP_RETRY_TIMEOUT_MS,
                    OPENHOP_ETH_DHCP_RETRY_RESPONSE_TIMEOUT_MS);
        }
#endif
    }
...
namespace TCPServer {
    ...
    void begin(uint16_t port, const String& token) {
        ...
        authenticated = false;
        parser.reset();
        listenerPort = port ? port : OPENHOP_ETH_TCP_PORT;
